    ...
    for m in members:
        ...
        if 0 <= delta_days <= days_ahead:
            upcoming.append({
                ...
                "turning_age": today.year - dob.year if delta_days > 0 or this_year_bday >= today else None,
            })
...
def get_pending_queue_for_user(user_id):
    """
    ...
    return (
        ...
        .filter_by(assigned_to=user_id, status="pending")
...
def get_admin_overview(church_id):
    ...
    return {
        ...
        "pending": sum(1 for r in records if r.follow_up_status == "not_started"),
        "completed": sum(1 for r in records if r.follow_up_status not in ("not_started", "not_applicable")),
