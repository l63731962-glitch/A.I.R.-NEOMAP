...
with app.app_context():
    ...
    # member_ids returned, cross-referenced against Member._store directly --
    # this doesn't depend on the (unfaked) relationship-loading machinery,
    # only on the real filtering logic in get_group_checkins_for_facilitator.
    ids_visible = {c.member_id for c in facilitator_view}
    names_visible = {m.full_name for m in Member._store if m.id in ids_visible}
    ...
    print(f"Facilitator sees after Alice leaves: {[m.full_name for m in Member._store if m.id in ids_after]}")
